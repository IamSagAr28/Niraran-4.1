...
export default function OrderSummary({items, onApplyCoupon}:{items:(Product & {qty:number})[]; onApplyCoupon:(code:string)=>void}){
  ...
  return (
    <aside className="bg-white p-4 rounded shadow-sm lg:sticky lg:top-28">
      ...
      <div className="space-y-2">
        ...
        <div className="border-t pt-3 mt-2">
          <div className="flex justify-between items-baseline">
            ...
            <span className="font-extrabold text-2xl text-[#2f5a3b]">₹{(subtotal + shipping + taxes).toFixed(2)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="my-4">
        {subtotal < goal ? (
          <div className="bg-amber-50 border-l-4 border-amber-400 text-amber-800 p-3 rounded">
            <div className="flex items-center gap-2">
              {/* celebratory icon */}
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-amber-600" viewBox="0 0 20 20" fill="currentColor" aria-hidden>
            ...
            <div className="mt-2 h-2 bg-gray-200 rounded overflow-hidden">
              <div className="h-2 bg-amber-400 rounded" style={{width: `${Math.round(progress * 100)}%`}} />
            </div>
            <div className="text-xs text-gray-600 mt-1">You're {Math.round(progress * 100)}% of the way to Free Shipping</div>
          </div>
        ) : (
          <div className="bg-green-50 border-l-4 border-green-400 text-green-800 p-3 rounded text-sm">You've unlocked Free Shipping — nice!</div>
        )}
      </div>
      ...
      <div className="mt-2">
        ...
        <div className="mt-2 flex gap-2">
          <input value={code} onChange={(e)=>setCode(e.target.value)} placeholder="Enter code" className="flex-1 rounded border border-[#B3C6A7] px-2 py-1" />
          <button onClick={()=>onApplyCoupon(code)} className="bg-[#48634A] text-white px-3 rounded hover:bg-[#3A503C] transition-colors">Apply</button>
        </div>
      </div>

      <div className="mt-4">
        <button className="w-full bg-[#48634A] text-white py-3 rounded hover:bg-[#3A503C] transition-colors">Proceed to Checkout</button>
      </div>
